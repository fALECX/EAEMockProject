...
import de.hnu.controller.AvailableRidesController;
...
/**
 * View für "Available Rides" (Page 120).
 *
 * Entspricht dem Balsamiq-Mockup:
 * - Header mit Back-Button, "Car Sharing", "Find a Ride"
 * - "Available rides:" Label mit Filter-Button
 * - Scrollbare Liste aller verfügbaren Fahrten
 * - Bottom Navigation Bar
 */
public class AvailableRidesView {

    private final BorderPane root;
    private final AvailableRidesController controller;

    public AvailableRidesView(AvailableRidesController controller) {
    ...
    /**
     * Erstellt den Hauptinhalt mit scrollbarer Ride-Liste.
     * ISO 9241-11 Effizienz: Alle Ergebnisse auf einen Blick, scrollbar.
     */
    private VBox createContent() {
        VBox content = new VBox(10);
        content.setPadding(new Insets(10, 15, 10, 15));

        // Titel-Zeile mit Filter
        ...
        // Filter-Button (Platzhalter)
        VBox filterBox = new VBox(2);
        filterBox.setAlignment(Pos.CENTER);
        Label filterIcon = new Label("🔻");
        filterIcon.setStyle("-fx-font-size: 18px;");
        Label filterLabel = new Label("Filter");
        filterLabel.setStyle("-fx-font-size: 11px; -fx-font-style: italic;");
        filterBox.getChildren().addAll(filterIcon, filterLabel);
        filterBox.setCursor(javafx.scene.Cursor.HAND);
        filterBox.setOnMouseClicked(e -> controller.onFilterClicked());

        titleRow.getChildren().addAll(titleLabel, spacer, filterBox);

        content.getChildren().add(titleRow);

        // ScrollPane für die Ride-Liste
        VBox rideList = new VBox(8);

        List<Ride> rides = controller.getAvailableRides();

        for (Ride ride : rides) {
            VBox rideCard = createRideCard(ride);
            rideList.getChildren().add(rideCard);
        }

        ScrollPane scrollPane = new ScrollPane(rideList);
     ...
     * ISO 9241-11 Zufriedenheit: Konsistentes Design mit Favoriten-View.
     */
    private VBox createRideCard(Ride ride) {
        ...
        topRow.getChildren().addAll(nameLabel, spacer, ratingBox);
